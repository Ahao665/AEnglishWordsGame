...
- 部署到 **GitHub Pages** 后，脚本与模型都从**你的站点**加载（不经过国外 CDN），**任何人**用电脑/手机打开链接即可使用，**无需 VPN**。
- 首次打开约需 10–30 秒加载 AI 模型，请勿关闭页面；之后会显示「识别已激活」。
- 请确保：① 仓库中已提交 `public/mediapipe/` 文件夹；② GitHub 仓库 **Settings → Pages → Source** 选择 **GitHub Actions**；③ 推送后等待 Actions 部署完成再访问。
